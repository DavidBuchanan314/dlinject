...
def dlinject(pid, lib_path, stopmethod="sigstop"):
    with open(f"/proc/{pid}/maps") as maps_file:
        for line in maps_file.readlines():
            libc_path = line.split()[-1]
            if re.match(r".*/libc.*\.so.*", libc_path):
                libc_base = int(line.split("-")[0], 16)
                break
        else:
            log_error("Couldn't find libc.so! (we need it for dl_open)")

    dlopen_func_name = "dlopen"
    log("libc.so found: " + repr(libc_path))
    log("libc.so base: " + hex(libc_base))
    dl_open_offset = lookup_elf_symbol(libc_path, dlopen_func_name)

    if not dl_open_offset:
        log_error("Unable to locate %s symbol" % dlopen_func_name)

    dl_open_addr = libc_base + dl_open_offset
